...
static unsigned int gpioLED1 = 49;           ///< Default GPIO for the LED is 13
module_param(gpioLED1, uint, S_IRUGO);       ///< Param desc. S_IRUGO can be read/not changed
MODULE_PARM_DESC(gpioLED1, " GPIO LED number (default=49)");     ///< parameter description
static unsigned int gpioLED2 = 117;           ///< Default GPIO for the LED is 13
module_param(gpioLED2, uint, S_IRUGO);       ///< Param desc. S_IRUGO can be read/not changed
MODULE_PARM_DESC(gpioLED2, " GPIO LED number (default=49)");     ///< parameter description

static unsigned int blinkPeriod1 = 1000;     ///< The blink period in ms
...
static unsigned int blinkPeriod2 = 5000;     ///< The blink period in ms
...
static bool ledOn1 = 0;                      ///< Is the LED on or off? Used for flashing
 ...
 */
static struct kobj_attribute period_attr = __ATTR(blinkPeriod1, 0660, period_show, period_store);
...
static int flash(void *arg){
   ...
   while(!kthread_should_stop()){           // Returns true when kthread_stop() is called
      ...
      if (mode==FLASH) ledOn1 = !ledOn1;      // Invert the LED state
      else if (mode==ON) ledOn1 = true;
      else ledOn1 = false;
      gpio_set_value(gpioLED1, ledOn1);       // Use the LED state to light/turn off the LED
...
static int __init ebbLED_init(void){
   ...
   gpio_direction_output(gpioLED1, ledOn1);   // Set the gpio to be in output mode and turn on
